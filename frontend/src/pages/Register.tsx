// MAKE THE TOAST WORK!!! (SHOULD BE EASY YOU HAVE DONE BEFORE)
...
const RegisterCard = () =>{
    ...
    const verifyInput = () => {
        ...
        if (emailRules.hasSpace.test(newUser.email)) {
            setFieldErr(cur => ({...cur, email: false}))
            return "email must be free of spaces"
        }
        ...
        if (!passwordRules.hasUppercase.test(newUser.password)) {
            setFieldErr(cur => ({...cur, password: false}))
            return "password must include an uppercase charachter"
        }
    ...
    const handleRegistration = (e?: React.FormEvent) => {
        ...
        const isInputVerified: string | null = verifyInput()
        if (isInputVerified) {
            let message = isInputVerified
            toast.error(message)
            // alert(message)
        }
        else {
            toast.success("yay it works")
            // alert("OK")
        }
