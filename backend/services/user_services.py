...
async def create_user(user: dict, session: Session = Depends(get_session)):
    ...
    '''
    # adding user ot db
    type_checked_user = User(**user)
    response = add_user_to_db(session, type_checked_user)
    ...
    # creates new token, hands token in to db ({email, tok} per dp)
    ...
    # extracting returned token
    returned_token = add_email_to_db_resp['token']
 
    # send the email with the token in the url
    response = await send_email_for_verify(returned_token, user["email"])
    if response["status_code"] != 200:
        return {"status_code":response["status_code"], "message":response['message']}
